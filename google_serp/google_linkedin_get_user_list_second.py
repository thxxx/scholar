...
if __name__ == "__main__":
    ...
    parser.add_argument("--keyword2", type=str, default="", required=True)
    args = parser.parse_args()

    keyword1 = args.keyword1
    keyword2 = args.keyword2

    filename = f"linkedin_search_results_{keyword1.replace(' ', '_')}_{keyword2.replace(' ', '_')}.csv"
    ...
    for idx in tqdm(range(0, int(100))):
        ...
        try:
            ...
            if idx == 1:
                print(f"\nStart for total_results: {keyword1} {keyword2} - {data}\n\n")
